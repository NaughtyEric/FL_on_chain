...
CPU fallback (no CUDA). Run from the repo root with the venv python:
    .venv/Scripts/python scripts/pretrain_model.py [--epochs 20] [--lr 0.1] \
        [--data-dir data/cifar100] [--out artifacts/pretrained_cifar100.npz]
...
def main() -> None:
    ...
    train_loader = DataLoader(
        load_cifar100(args.data_dir, split="train"), batch_size=args.batch_size, shuffle=True, num_workers=0
    )
    test_loader = DataLoader(
        load_cifar100(args.data_dir, split="test"), batch_size=256, shuffle=False, num_workers=0
    )
